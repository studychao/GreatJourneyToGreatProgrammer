...
TCP三次握手过程🤝
第一次握手： 主机A（客户端） 向主机B（服务器） 发送一个含有同步序列号（SYN）的标志位的数据段给主机B，向主机B请求建立连接，通过这个数据段，主机A告诉主机B两件事：我要和你通信；你可以用哪个序列号作为起始数据段来回应，此时将SYN位置设为1，SequenceNumber = x,然后A进入SYN_send模式
...
2） 数据链路层
数据链路层负责物理寻址、错误侦测和改错。当表头和表尾被加至数据包时，会形成帧。
【MAC地址】
